...
def excel_original_data(concatenated_df):
    ...
    all_indicators = pd.concat([debt_payment, profit_ability, operation_ability, development_ability,
                                enterprise_scale, innovation_ability, operation_condition, enterprise_credit], axis=1)
...
def final_score(eight_capability):
    ...
    # 计算最终得分
    final_score = ((weight_score['score'] - average_weight) / (weight_score['score'].max() - average_weight) * 0.15 + 0.85) * 100
...
with open('model1.json', 'w', encoding='utf-8') as json_file:
