...
def evaluate(data, total, group):
    def start_dataframe(json_data, columns):

        company_name_list = []

        for i in range(int(columns)):
            company_name = json_data[i]['company_name']
    ...
    # 公司名称和全量数据
    company_name_df, concatenated_df = start_dataframe(data, total)
    ...
    def calculate_solution(calculated_data, indicator_weights):
        ...
        # 计算最终得分
        final_score = ((proximity_score - average_weight) / (
                    proximity_score.max() - average_weight) * 0.15 + 0.85) * 100
